...
- 워크스페이스에 `_planu_extract.py`가 없거나 현재 파일 구조와 맞지 않으면, 먼저 로컬 파일에서 헤더와 첫 3~5줄만 소량 검사해 `_planu_extract.py`를 자동 작성한다. 이 소량 검사는 전체 데이터 직접 로드가 아니라 스크립트 부트스트랩 단계로 허용한다.
